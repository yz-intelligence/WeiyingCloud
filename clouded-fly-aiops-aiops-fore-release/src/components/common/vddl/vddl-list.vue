...
export default {
  ...
  methods: {
    ...
    handleDragover(event) {
      ...
      if (event.target !== this.listNode) {
        ...
        if (this.isMouseInFirstHalf(event, this.placeholderNode, true)) {
          ...
          while (
            this.placeholderNode.previousElementSibling &&
          ...
          while (
            this.placeholderNode.nextElementSibling &&
            !this.isMouseInFirstHalf(event, this.placeholderNode.nextElementSibling, true)
          ) {
            this.listNode.insertBefore(this.placeholderNode, this.placeholderNode.nextElementSibling.nextElementSibling)
          }
        }
      }
      ...
      if (this.$el.className.indexOf('vddl-dragover') < 0)
        this.$el.className = this.$el.className.trim() + ' vddl-dragover'
      return false
    },
    handleDrop(event) {
      if (!this.isDropAllowed(event)) return true
    ...
    isMouseInFirstHalf(event, targetNode, relativeToParent) {
      var mousePointer = this.horizontal ? event.offsetX || event.layerX : event.offsetY || event.layerY
